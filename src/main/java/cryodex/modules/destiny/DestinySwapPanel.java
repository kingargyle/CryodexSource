...
public class DestinySwapPanel extends JPanel {
	...
	public static void showSwapPanel() {
		...
		panel.add(new DestinySwapPanel(manualModificationPanel), BorderLayout.CENTER);
	...
	private final DestinyPlayer blankPlayer = new DestinyPlayer(new Player());
	...
	public DestinySwapPanel(JDialog parent) {
		super(new BorderLayout());

		this.parent = parent;

		matches = new ArrayList<>();
		matches.addAll(((DestinyTournament) CryodexController.getActiveTournament())
	...
	public JPanel getQuickEntryPanel() {
		if (quickEntryPanel == null) {
			...
			playerList.addAll(((DestinyTournament) CryodexController
					.getActiveTournament()).getDestinyPlayers());
		...
		if (swapButton == null) {
			...
			swapButton.addActionListener(new ActionListener() {
				...
				public void actionPerformed(ActionEvent e) {
					...
					DestinyRound r = new DestinyRound(matches, tournament, roundNumber);
	...
	private JButton getAddButton() {
		if (addButton == null) {
			...
			addButton.addActionListener(new ActionListener() {
				...
				@Override
				public void actionPerformed(ActionEvent e) {
					MatchPanel mp = new MatchPanel(new DestinyMatch(), matchPanels
							.size());
	...
	private void updatePanels() {
		...
		List<DestinyPlayer> tempPlayers = new ArrayList<>();
		tempPlayers.addAll(((DestinyTournament) CryodexController
				.getActiveTournament()).getDestinyPlayers());
	...
	private class MatchPanel extends JPanel {
		...
		private JComboBox<DestinyPlayer> player1Combo;
		private JComboBox<DestinyPlayer> player2Combo;

		private final DestinyMatch match;

		public MatchPanel(DestinyMatch match, int tableNumber) {
		...
		public DestinyMatch getNewMatch() {
			DestinyPlayer p1 = (DestinyPlayer) getPlayer1Combo().getSelectedItem();
			DestinyPlayer p2 = (DestinyPlayer) getPlayer2Combo().getSelectedItem();
			...
			DestinyMatch m = new DestinyMatch(p1, p2);
		...
		public JComboBox<DestinyPlayer> getPlayer1Combo() {

			if (player1Combo == null) {
				player1Combo = new JComboBox<DestinyPlayer>();
		...
		public JComboBox<DestinyPlayer> getPlayer2Combo() {

			if (player2Combo == null) {
				player2Combo = new JComboBox<DestinyPlayer>();
		...
		public void updateCombos(List<DestinyPlayer> players) {
			...
			List<DestinyPlayer> list1 = new ArrayList<>();
			List<DestinyPlayer> list2 = new ArrayList<>();
