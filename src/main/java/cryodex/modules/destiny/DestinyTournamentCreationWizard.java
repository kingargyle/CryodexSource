...
import cryodex.modules.destiny.DestinyTournament.InitialSeedingEnum;
import cryodex.widget.ComponentUtils;
import cryodex.widget.DoubleList;
import cryodex.widget.SpringUtilities;

public class DestinyTournamentCreationWizard extends JDialog {
	...
	public DestinyTournamentCreationWizard() {
		super(Main.getInstance(), "Tournament Wizard", true);

		this.add(getMainPanel());
		setCurrentPage(new MainPage());
		DestinyTournamentCreationWizard.this.pack();
	...
	private void goToPrevious() {
		...
		if (getPreviousPage() != null) {
			...
			DestinyTournamentCreationWizard.this.setVisible(false);
		}
	}
	...
	private class PlayerSelectionPage implements Page {
		...
		@Override
		public void onNext() {
			List<DestinyPlayer> iaPlayerList = new ArrayList<>();
			for (Player p : playerList.getList2Values()) {
				iaPlayerList.add((DestinyPlayer) p.getModuleInfoByModule(Modules.DESTINY
						.getModule()));
			}
		...
		public void onFinish() {
			...
			if (wizardOptions.getSplit() > 1) {
				...
				for (int i = 1; i <= splitNum; i++) {
					...
					newWizardOption.setPlayerList(new ArrayList<DestinyPlayer>());
				}
				...
				if (splitByGroupRB.isSelected()) {
					...
					// Add players to map
					for (DestinyPlayer p : wizardOptions.getPlayerList()) {
					...
					for (String groupValue : playerMap.keySet()) {
						...
						List<DestinyPlayer> playerList = playerMap.get(groupValue);
							...
							if (wizardOptionsList.get(last).getPlayerList()
								...
								if (wizardOptionsList.get(first)
										...
										&& wizardOptionsList.get(last)
												.getPlayerList().size() % 2 == 1) {
									DestinyPlayer p = wizardOptionsList
											.get(first)
											.getPlayerList()
					...
					List<DestinyPlayer> tempPlayers = rankMergedPlayers(wizardOptions
							.getPlayerList());
					...
					for (int i = 0; i < wizardOptionsList.size(); i++) {
						...
						while (i + 1 < wizardOptionsList.size()
							...
							DestinyPlayer t1 = wizardOptionsList
									.get(i)
									.getPlayerList()
									.get(wizardOptionsList.get(i)
											.getPlayerList().size() - 1);
							wizardOptionsList.get(i).getPlayerList().remove(t1);
							List<DestinyPlayer> temp = new ArrayList<DestinyPlayer>();
					...
					List<DestinyPlayer> playerList = wizardOptions.getPlayerList();
				...
				for (WizardOptions wo : wizardOptionsList) {
					if (wo.getInitialSeedingEnum() == InitialSeedingEnum.IN_ORDER) {
						List<DestinyPlayer> tempList = rankMergedPlayers(wo
								.getPlayerList());
						wo.setPlayerList(tempList);
					}
					DestinyModule.makeTournament(wo);
				}
			} else {
				DestinyTournamentCreationWizard.this.setVisible(false);
				DestinyModule.makeTournament(wizardOptions);
			}
		}
	}
	...
	private class MergeTournamentSelectionPage implements Page {
		...
		public void onNext() {
			...
			List<DestinyTournament> tournamentList = new ArrayList<DestinyTournament>();
			Set<DestinyPlayer> playerList = new TreeSet<DestinyPlayer>();
			...
			for (DestinyTournament t : checkBoxMap.keySet()) {
				if (checkBoxMap.get(t).isSelected()) {
					...
					List<DestinyPlayer> thisTournamentPlayers = new ArrayList<DestinyPlayer>();
					thisTournamentPlayers.addAll(t.getDestinyPlayers());
					...
					if (playerCount == null
						...
						Collections
								.sort(thisTournamentPlayers, new DestinyComparator(
										t, DestinyComparator.rankingCompare));
			...
			List<DestinyPlayer> addingList = new ArrayList<DestinyPlayer>();
	...
	public static class WizardOptions {
		...
		private List<DestinyPlayer> playerList;
		private int split = 1;
		private boolean isMerge = false;
		private List<DestinyTournament> selectedTournaments;
		...
		public List<DestinyPlayer> getPlayerList() {
			return playerList;
		}

		public void setPlayerList(List<DestinyPlayer> playerList) {
		...
		public List<DestinyTournament> getSelectedTournaments() {
			return selectedTournaments;
		}

		public void setSelectedTournaments(
				List<DestinyTournament> selectedTournaments) {
	...
	public List<DestinyPlayer> rankMergedPlayers(List<DestinyPlayer> playerList) {
		DestinyTournament mergeTournament = new DestinyTournament("",
				wizardOptions.getPlayerList(), null, false);
		for (DestinyTournament t : wizardOptions.getSelectedTournaments()) {
			mergeTournament.getAllRounds().addAll(t.getAllRounds());
		}

		List<DestinyPlayer> tempPlayers = new ArrayList<DestinyPlayer>();
		tempPlayers.addAll(playerList);
		Collections.sort(tempPlayers, new DestinyComparator(mergeTournament,
